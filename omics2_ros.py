...
def prediction(mRNA_value,GAN_epoch,labels,update=None):
  ...
  if update==1:
    print('AUC for real omics2:', np.mean(AUC_mean,axis=0))
    xxx=pd.DataFrame(np.array(AUC_all)).transpose()
    xxx.to_csv(curdir+'real_omics2_AUC.inc', sep='\t')
    x=pd.DataFrame(np.array(ACC_all)).transpose()
    x.to_csv(curdir+'real_omics2_ACC.inc', sep='\t')    
    xx=pd.DataFrame(np.array(F1_all)).transpose()
    xx.to_csv(curdir+'real_omics2_F1.inc', sep='\t')   
  '''
  if GAN_epoch==0:
...
def omics2(update,omics1,omics2,omics3,adj_file,adj_file3,label):
    ...
    yz, y1_ind, z_ind = np.intersect1d(miRNA.columns,meth.columns,return_indices=True)
    _, y1_ind3, z_ind1 = np.intersect1d(meth.index,adj3.columns,return_indices=True)
    yz1, y1_ind4, z_ind2 = np.intersect1d(miRNA.index.values,adj3.index.values,return_indices=True)
    ...
    meth = meth.iloc[y_ind1,:]
    adj3 = adj3.iloc[:,z_ind1]
    adj3 = adj3.iloc[z_ind2,:]
    meth = meth.fillna(0)
    ...
    xy, x_ind, y_ind = np.intersect1d(miRNA.columns,data.index,return_indices=True)
    mRNA = mRNA.iloc[:,x_ind]
...
############    
    ...
    for epoch in range(num_epochs):
        for n, (real_samples, _) in enumerate(miRNA_train_loader):
            ...
            latent_value = (torch.matmul(mRNA_train_data.t(),adj)+torch.matmul(meth_train_data.t(),adj3))/2
